...
import { McapIndexedReader } from "./McapIndexedReader.js";
import { McapRecordBuilder } from "./McapRecordBuilder.js";
import { McapWriter } from "./McapWriter.js";
import { TempBuffer } from "./TempBuffer.js";
import { MCAP_MAGIC, Opcode } from "./constants.js";
import { record, uint64LE, uint32LE, string, keyValues, collect, uint16LE, uint32PrefixedBytes, } from "./testUtils.js";
/**
    ...
    return {
        ...
        read: async (offset, size) => {
            ...
            if (offset < 0 || size < 0 || offset + size > data.length) {
                throw new Error(`Read out of range: offset ${offset}, size ${size} (data.length: ${data.length})`);
            }
            reusableBuffer.set(new Uint8Array(data.buffer, data.byteOffset + Number(offset), Number(size)));
            reusableBuffer.fill(0xff, Number(size));
            return new Uint8Array(reusableBuffer.buffer, 0, Number(size));
        },
    };
}
function writeChunkWithMessageIndexes(builder, chunk) {
    ...
    const chunkMessageIndexOffsets = new Map();
...
describe("McapIndexedReader", () => {
    it("rejects files that are too small", async () => {
        await expect(McapIndexedReader.Initialize({
            readable: makeReadable(new Uint8Array([
                ...MCAP_MAGIC,
    ...
    it("rejects unindexed file", async () => {
        const readable = makeReadable(new Uint8Array([
            ...MCAP_MAGIC,
            ...record(Opcode.HEADER, [
                ...string(""), // profile
                ...string(""), // library
    ...
    it("includes library in error messages", async () => {
        ...
        await expect(McapIndexedReader.Initialize({ readable })).rejects.toThrow("File is not indexed [library=lib]");
    });
    it("rejects invalid index crc", async () => {
        ...
        data.push(...record(Opcode.METADATA, [
            ...string("foobar"),
            ...keyValues(string, string, []), // metadata
        ]));
        data.push(...record(Opcode.FOOTER, [
        ...
        await expect(McapIndexedReader.Initialize({ readable })).rejects.toThrow("Incorrect summary CRC 1656343536 (expected 163128923)");
    });
    it("parses index with schema and channel", async () => {
        ...
        const summaryStart = data.length;
        data.push(...record(Opcode.SCHEMA, [
        ...
        expect(reader.channelsById).toEqual(new Map([
            [
                42,
                {
                    type: "Channel",
        ...
        expect(reader.schemasById).toEqual(new Map([
            [
                1,
                {
                    type: "Schema",
    ...
    describe("indexed with single channel", () => {
        ...
        it.each([
        ...
        ])("fetches chunk data and reads requested messages between $startTime and $endTime", async ({ startTime, endTime, expected }) => {
            ...
            data.push(...record(Opcode.MESSAGE_INDEX, [
                ...uint16LE(42), // channel id
            ...
            const summaryStart = data.length;
            data.push(...channel, ...record(Opcode.CHUNK_INDEX, [
            ...
            }
            {
                const readable = makeReadable(new Uint8Array(data));
                const reader = await McapIndexedReader.Initialize({ readable });
                const collected = await collect(reader.readMessages({ startTime, endTime, reverse: true }));
    ...
    describe("indexed with multiple channels", () => {
        ...
        it.each([
        ...
        ])("fetches chunk data and reads requested messages between $startTime and $endTime", async ({ startTime, endTime, expectedIndices }) => {
            ...
            await writer.end();
            {
                ...
                const collected = await collect(reader.readMessages({ startTime, endTime }));
                expect(collected).toEqual(expectedIndices.map((i) => ({ channelId: channelIds[i], ...messages[i] })));
            }
            {
                const reader = await McapIndexedReader.Initialize({ readable: tempBuffer });
                const collected = await collect(reader.readMessages({ startTime, endTime, reverse: true }));
                expect(collected).toEqual(expectedIndices.map((i) => ({ channelId: channelIds[i], ...messages[i] })).reverse());
            }
        });
    });
    ...
    it("caches message indexes across readMessages() calls when messageIndexCacheSizeBytes > 0", async () => {
        ...
        const overlapsMessageIndex = ({ offset, size }) => messageIndexRanges.some(({ start, end }) => offset < end && offset + size > start);
    ...
    it("re-reads message indexes across readMessages() calls when messageIndexCacheSizeBytes is not set", async () => {
        ...
        const overlapsMessageIndex = ({ offset, size }) => offset < messageIndexRange.end && offset + size > messageIndexRange.start;
    ...
    it("reads metadata records", async () => {
        ...
        data.push(...record(Opcode.DATA_END, [
            ...uint32LE(0), // data crc
        ]));
        const summaryStart = data.length;
        data.push(...record(Opcode.METADATA_INDEX, [
    ...
    it("reads attachment records", async () => {
        ...
        data.push(...record(Opcode.ATTACHMENT, [
        ...
        const summaryStart = data.length;
        data.push(...record(Opcode.ATTACHMENT_INDEX, [
...
//# sourceMappingURL=McapIndexedReader.test.js.map