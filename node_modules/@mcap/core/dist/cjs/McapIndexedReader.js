...
exports.McapIndexedReader = void 0;
const crc_1 = require("@foxglove/crc");
const heap_js_1 = require("heap-js");
const CachedReadable_ts_1 = require("./CachedReadable.js");
const ChunkCursor_ts_1 = require("./ChunkCursor.js");
const Reader_ts_1 = require("./Reader.js");
const constants_ts_1 = require("./constants.js");
const parse_ts_1 = require("./parse.js");
class McapIndexedReader {
    ...
    constructor(args) {
        ...
        this.#messageIndexReadable =
            messageIndexCacheSizeBytes > 0
                ? new CachedReadable_ts_1.CachedReadable(this.#readable, messageIndexCacheSizeBytes)
                : this.#readable;
        ...
        for (const attachment of args.attachmentIndexes) {
            if (this.#attachmentStartTime == undefined ||
    ...
    static async Initialize({ readable, decompressHandlers, messageIndexCacheSizeBytes, }) {
        ...
        let headerEndOffset;
        {
            const headerPrefix = await readable.read(0n, BigInt(constants_ts_1.MCAP_MAGIC.length + /* Opcode.HEADER */ 1 + /* record content length */ 8));
            const headerPrefixView = new DataView(headerPrefix.buffer, headerPrefix.byteOffset, headerPrefix.byteLength);
            void (0, parse_ts_1.parseMagic)(new Reader_ts_1.default(headerPrefixView));
            const headerContentLength = headerPrefixView.getBigUint64(constants_ts_1.MCAP_MAGIC.length + /* Opcode.HEADER */ 1, true);
            const headerReadLength = 
            /* Opcode.HEADER */ 1n + /* record content length */ 8n + headerContentLength;
            const headerRecord = await readable.read(BigInt(constants_ts_1.MCAP_MAGIC.length), headerReadLength);
            headerEndOffset = BigInt(constants_ts_1.MCAP_MAGIC.length) + headerReadLength;
            const headerReader = new Reader_ts_1.default(new DataView(headerRecord.buffer, headerRecord.byteOffset, headerRecord.byteLength));
            const headerResult = (0, parse_ts_1.parseRecord)(headerReader, true);
        ...
        {
            const headerLengthLowerBound = BigInt(constants_ts_1.MCAP_MAGIC.length +
                /* Opcode.HEADER */ 1 +
            ...
            /* Opcode.FOOTER */ 1 +
                ...
                /* crc */ 4 +
                constants_ts_1.MCAP_MAGIC.length);
        ...
        try {
            void (0, parse_ts_1.parseMagic)(new Reader_ts_1.default(footerAndMagicView, footerAndMagicView.byteLength - constants_ts_1.MCAP_MAGIC.length));
        }
        catch (error) {
        ...
        let footer;
        {
            const footerReader = new Reader_ts_1.default(footerAndMagicView);
            const footerRecord = (0, parse_ts_1.parseRecord)(footerReader, true);
            ...
            if (footerReader.bytesRemaining() !== constants_ts_1.MCAP_MAGIC.length) {
        ...
        if (footer.summaryCrc !== 0) {
            let summaryCrc = (0, crc_1.crc32Init)();
            summaryCrc = (0, crc_1.crc32Update)(summaryCrc, dataEndAndSummarySection.subarray(Number(dataEndLength)));
            summaryCrc = (0, crc_1.crc32Update)(summaryCrc, footerPrefix);
            summaryCrc = (0, crc_1.crc32Final)(summaryCrc);
        ...
        const indexReader = new Reader_ts_1.default(indexView);
        ...
        let first = true;
        let result;
        while ((result = (0, parse_ts_1.parseRecord)(indexReader, true))) {
    ...
    async *readMessages(args = {}) {
        ...
        const chunkCursors = new heap_js_1.Heap((a, b) => a.compare(b));
        ...
        for (const chunkIndex of this.chunkIndexes) {
            if (chunkIndex.messageStartTime <= endTime && chunkIndex.messageEndTime >= startTime) {
                chunkCursors.push(new ChunkCursor_ts_1.ChunkCursor({
                    chunkIndex,
                    relevantChannels,
                    startTime,
                    endTime,
        ...
        const chunkReader = new Reader_ts_1.default(new DataView(new ArrayBuffer(0)));
        for (let cursor; (cursor = chunkCursors.peek());) {
            ...
            const record = (0, parse_ts_1.parseRecord)(chunkReader, validateCrcs ?? true);
            if (!record) {
    ...
    async *readMetadata(args = {}) {
        ...
        for (const metadataIndex of this.metadataIndexes) {
            ...
            const metadataReader = new Reader_ts_1.default(new DataView(metadataData.buffer, metadataData.byteOffset, metadataData.byteLength));
            const metadataRecord = (0, parse_ts_1.parseRecord)(metadataReader, false);
    ...
    async *readAttachments(args = {}) {
        ...
        for (const attachmentIndex of this.attachmentIndexes) {
            ...
            const attachmentReader = new Reader_ts_1.default(new DataView(attachmentData.buffer, attachmentData.byteOffset, attachmentData.byteLength));
            const attachmentRecord = (0, parse_ts_1.parseRecord)(attachmentReader, validateCrcs ?? true);
    ...
    async #loadChunkData(chunkIndex, options) {
        ...
        const chunkReader = new Reader_ts_1.default(new DataView(chunkData.buffer, chunkData.byteOffset, chunkData.byteLength));
        const chunkRecord = (0, parse_ts_1.parseRecord)(chunkReader, options?.validateCrcs ?? true);
        ...
        if (chunk.uncompressedCrc !== 0 && options?.validateCrcs !== false) {
            const chunkCrc = (0, crc_1.crc32)(buffer);
...
exports.McapIndexedReader = McapIndexedReader;
