...
exports.McapRecordBuilder = void 0;
const crc_1 = require("@foxglove/crc");
const BufferBuilder_ts_1 = require("./BufferBuilder.js");
 ...
 */
class McapRecordBuilder {
    options;
    #bufferBuilder = new BufferBuilder_ts_1.BufferBuilder();
    constructor(options) {
    ...
    writeMagic() {
        this.#bufferBuilder.bytes(new Uint8Array(constants_ts_1.MCAP_MAGIC));
    }
    writeHeader(header) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.HEADER);
    ...
    writeFooter(footer) {
        this.#bufferBuilder
            .uint8(constants_ts_1.Opcode.FOOTER)
    ...
    writeSchema(schema) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.SCHEMA);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
    ...
    writeChannel(info) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.CHANNEL);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder
            .uint16(info.id)
    ...
    writeMessage(message) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.MESSAGE);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder
    ...
    writeAttachment(attachment) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.ATTACHMENT);
        const startPosition = this.#bufferBuilder.length;
        ...
        this.#bufferBuilder.uint32((0, crc_1.crc32)(this.#bufferBuilder.bufferView(crcStartPosition, this.#bufferBuilder.length - crcStartPosition)));
        if (this.options?.padRecords === true) {
            this.#bufferBuilder.uint8(0x01).uint8(0xff).uint8(0xff);
        }
        const endPosition = this.#bufferBuilder.length;
    ...
    writeAttachmentIndex(attachmentIndex) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.ATTACHMENT_INDEX);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder
    ...
    writeChunk(chunk) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.CHUNK);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder
    ...
    writeChunkIndex(chunkIndex) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.CHUNK_INDEX);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder
    ...
    writeMessageIndex(messageIndex) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.MESSAGE_INDEX);
        const startPosition = this.#bufferBuilder.length;
    ...
    writeMetadata(metadata) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.METADATA);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder size
    ...
    writeMetadataIndex(metadataIndex) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.METADATA_INDEX);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder size
    ...
    writeSummaryOffset(summaryOffset) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.SUMMARY_OFFSET);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder size
    ...
    writeStatistics(statistics) {
        this.#bufferBuilder.uint8(constants_ts_1.Opcode.STATISTICS);
        const startPosition = this.#bufferBuilder.length;
        this.#bufferBuilder
            .uint64(0n) // placeholder size
    ...
    writeDataEnd(dataEnd) {
        this.#bufferBuilder
            .uint8(constants_ts_1.Opcode.DATA_END)
...
exports.McapRecordBuilder = McapRecordBuilder;
//# sourceMappingURL=McapRecordBuilder.js.map