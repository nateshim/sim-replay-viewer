...
function parseMagic(reader) {
    if (reader.bytesRemaining() < constants_ts_1.MCAP_MAGIC.length) {
        return undefined;
    }
    const magic = reader.u8ArrayBorrow(constants_ts_1.MCAP_MAGIC.length);
    if (!constants_ts_1.MCAP_MAGIC.every((val, i) => val === magic[i])) {
        throw new Error(`Expected MCAP magic '${constants_ts_1.MCAP_MAGIC.map((val) => val.toString(16).padStart(2, "0")).join(" ")}', found '${Array.from(magic, (_, i) => magic[i].toString(16).padStart(2, "0")).join(" ")}'`);
    }
...
function parseAttachment(reader, recordLength, 
// NOTE: internal function in the hot path, (de)structuring  args would be wasteful, acceptable perf/clarity tradeoff
// eslint-disable-next-line @foxglove/no-boolean-parameters
validateCrcs) {
    const startOffset = reader.offset;
    ...
    if (validateCrcs && expectedCrc !== 0) {
        ...
        const actualCrc = (0, crc_1.crc32)(fullData);
