...
exports.generateBindgenConfig = generateBindgenConfig;
...
function shouldGenerateRustType(schema) {
    return schema.name !== "Timestamp" && schema.name !== "Duration";
}
function rustEnumName(name) {
 ...
 */
function rustMessageSchemaName(schema) {
...
function assertValidBindgen(bindgen) {
    if (typeof bindgen.export !== "object" ||
 ...
 */
async function generateBindgenConfig(schemas, enums, preludeFile) {
    const comment = `
#
# Generated by https://github.com/foxglove/foxglove-sdk
#
...
#
`;
    const prelude = await promises_1.default.readFile(preludeFile, "utf-8");
    const bindgenToml = (0, smol_toml_1.parse)(prelude);
    ...
    return comment + "\n" + (0, smol_toml_1.stringify)(bindgenToml);
}
